...
function writeFile(path, ext, data, options,callback) {
  ...
  fs.readdir(path, function(err) {
    ...
    fs.access(fileName, function(errF) { // check file exists
      if(errF) fs.writeFileSync(fileName, data);  // if file no exist, write file
      if(options.overwrite) { // if user wants to overwrite
        fs.access(fileName, fs.W_OK, function(errW) {  // check file is writeable
          if(errW) throw errW;
